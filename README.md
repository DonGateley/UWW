...
How to get data associated with a CAIN across systems.  How to create that in the first place.  The amount of memory an event can have associated is arbitrary.  A system function to bind existing memory to events is needed.  

Cain1.CainBind(data) bind an event to a workspace copy of a block of computer data.  The amount of bound memory can be virtually without limit.  The data associated with an event is immutable.  Hashing comes through again.
...
The existence statement: By commiting the JavaScript CainBang.js file at GitHub I have brought into existence the Cain Space.  Any information that can be gleaned from that creation may be used in any way, or not, to form "CainZero", the consensus (relative to some CMB) event in Cain Space that represents zero in it.  Some transformation of that point can and will then be used to commit the event "CainZero" (CainZero.js) once and for all time.  As yet that doesn't exist and is under consideration.  When published on GitHub, the event CainZero becomes immutable, distributable and universal by decree.  Copyright is hereby expressly forbidden for the subsequent name, CainZero, or the number association.  Events in Cain Space are very far appart and unlikely to collide.  The bits of CainZero itself will appear as a very large random number (which might be as good as anything.  Even the best thing.) I need to think more and to continue to learn the node interpreter before actually doing that.  (Although those who learn to think in this space realize its already been done.) 
